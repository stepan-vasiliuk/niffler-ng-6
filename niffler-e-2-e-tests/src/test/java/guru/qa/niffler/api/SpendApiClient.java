...
public class SpendApiClient {
    ...
    private final Retrofit retrofit = new Retrofit.Builder().baseUrl(Config.getInstance().spendUrl()).addConverterFactory(JacksonConverterFactory.create()).build();
    ...
    public List<SpendJson> getSpends(String userName, CurrencyValues currencyValues, String fromDate, String toDate) {
        final Response<List<SpendJson>> response;
        try {
            response = spendApi.getSpends(userName, currencyValues, fromDate, toDate).execute();
        } catch (IOException e) {
            throw new AssertionError(e);
        }
        assertEquals(200, response.code());
    ...
    public CategoryJson addCategory(CategoryJson category) {
        ...
        try {
            response = spendApi.addNewCategory(category).execute();
        } catch (IOException e) {
            throw new AssertionError(e);
        }
        assertEquals(200, response.code());
